...
class GenkiShikenFragment : Fragment(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private inner class QuestionViewHolder(private val mItemView: View) :
         ...
         *
         * @throws IllegalStateException If quiz is not testing meaning or reading.
        ...
        fun toggleSingleRadioButtonText(rb: RadioButton, index: Int) {
            rb.text = when (mLessonType) {
                LessonType.KANJI_MEANING -> mQuestion?.mShuffledCards?.get(index)?.meaning
                LessonType.KANJI_READING -> mQuestion?.mShuffledCards?.get(index)?.onYomi +
                        " / " +
                        mQuestion?.mShuffledCards?.get(index)?.kunYomi
                else -> throw IllegalStateException(
                        "Can only set RadioButton text to meaning or reading.")
            }
        }
